...
public interface DetailKhachSanMapper {
    ...
    @Mapping(target = "soPhongKinhDoanh",
            expression = "java(ks.getPhongs() != null ? ks.getPhongs().size() : 0)")
    @Mapping(target = "soNhanVien",
            expression = "java(ks.getNhanViens() != null ? ks.getNhanViens().size() : 0)")
    @Mapping(target = "soBaiDangPhong",
            expression = "java(ks.getBaiDangPhongs() != null ? ks.getBaiDangPhongs().size() : 0)")
    @Mapping(target = "danhGiaTrungBinh",
            expression = "java(tinhDiemTrungBinh(ks))")
    @Mapping(target = "hinhAnh", source = "hinhAnh")

    DetailKhachSanResponse toDetailResponse(KhachSan ks);

    // map HinhAnh → String
    default String map(HinhAnh ha) {
        return ha.getImageUrl();
    }
